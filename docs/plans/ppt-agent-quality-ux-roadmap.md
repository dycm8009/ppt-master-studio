...
3. 重大叙事与表达问题能在全面制作前被用户识别。
4. 局部修改保留明确的不变量，只重建受影响的工件。
5. 任何正向完成状态都能追溯到当前版本的官方证据。
6. 实验工具不冒充生产质量 Gate；质量提升须有基线与评测数据支持。
7. 交付说明区分已导出、已在目标 PowerPoint 验证和未验证能力。

---

## 9. 实施与验收状态

本计划仍为分阶段实施，不表示全部完成。PR #34 包含基础实现及 QA 修复，尚未合并到 `studio-main`。

| 工作流 | 实施状态 | 尚缺内容 |
| --- | --- | --- |
| WS-A 可靠状态 | 部分实现；状态读取复用 Harness，修复过期证据的正向误报 | Hosted 前端统一呈现、自动回传闭环、完整状态事件基线 |
| WS-B 故事板 | 已有只读投影与完整性检查 | 用户验证和生成后结构性返工指标 |
| WS-C 设计方向 | 未实施新的内容绑定 UI | 三方向内容对比与用户评测 |
| WS-D 代表页 | 仅实验选择器 | 固定语料、对照实验、启用阈值；未改变生成节奏 |
| WS-E 语义 QA | 仅结构/文本启发式实验 | 事实支持、视觉拓扑、人工盲评与准确率基线 |
| WS-F 增量审阅 | 页面变化提示与当前版本重验 | 修改范围模型、完整依赖追踪、合法的批准复用 |
| WS-G PPTX 验证 | 未新增目标 PowerPoint 验证 | 目标环境测试与交付能力摘要 |
| WS-H 工作区 | 未实施统一项目工作区 | 导航、统一版本显示与交互验证 |

本次 QA 记录见 [实施质量检查与修复记录](ppt-agent-quality-ux-qa.md)。
