...
9. **Default Generate Storyboard projection** — For `Generate PPTX — ordinary Default`, after Design Spec Gate 1 and the current lock Gate 2 pass, read and run [`workflows/stages/storyboard.md`](workflows/stages/storyboard.md) as a non-blocking derived view before Executor. It projects the current Design Spec only; it creates no confirmation receipt, does not reopen a gate, and must be rebuilt before re-presentation after Design Spec mutation.
