...
While the deck is being generated, open the exact URL reported by the launcher in Cloud Browser. It is formatted as `http://terminal.local:<port>`; live preview prefers port `6060` and uses the next free port when it is occupied.
